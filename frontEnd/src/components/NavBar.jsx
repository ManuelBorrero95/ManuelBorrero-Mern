    ...
    <nav className="flex justify-between items-center px-10 py-5 h-[100px] text-black dark:text-white bg-white dark:bg-black fixed top-0 w-full z-50 transition-colors duration-300 border-b border-gray-200 dark:border-white/30 ">
      ...
      {isLoggedIn ? (
        <>
          ...
          <div>
            <Link to="/create">
              <button className="border-2 border-solid border-transparent text-xl font-bold hover:border-Viola rounded-lg p-[5px] md:p-[10px] md:px-4 font-mono transition-all duration-300 ease-in-out">
                + New Post
              </button>
            </Link>
          </div>
        ...
        <div className="flex gap-4 justify-center items-center text-black dark:text-white font-mono">
          <Link className="hover:text-Viola" to="/login">Login</Link>
          <Link className="hover:text-Viola" to="/register">Register</Link>
        </div>
      )}
    </nav>
  );
